/** Deterministic local integration fixture; never touches a production directory. */
...
const sid = createStreamer({ name: '原版 Pi 兼容性测试', slug: 'hybrid-pi', biliUid: '969696', roomId: '969696', enabled: false });
...
getDb().prepare("UPDATE media_assets SET local_path='hybrid.gif',mime_type='image/gif',byte_size=?,state='stored' WHERE id=?").run(image.length, asset.id);
