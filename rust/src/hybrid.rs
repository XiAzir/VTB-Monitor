//! Original SvelteKit/TypeScript Pi adapter. The helper is demand-started and
//! reaped while idle; every helper request owns a lease through response EOF.
...
use std::{io, path::PathBuf, process::Stdio, sync::{Arc, atomic::{AtomicUsize, AtomicU64, Ordering}}, time::{Duration, Instant}};
...
pub struct Bridge {
    ...
    root: PathBuf, node: String, idle: Duration, pub client: reqwest::Client,
}
impl Bridge {
    pub fn new() -> Result<Arc<Self>> {
        let root = std::env::var("VTBM_APP_ROOT").map(PathBuf::from).unwrap_or(std::env::current_dir()?);
        ...
        let this = Arc::new(Self {
            ...
            root, node: std::env::var("VTBM_NODE").unwrap_or_else(|_| "node".into()), idle: Duration::from_millis(idle_ms),
    ...
    pub async fn run_job(self: &Arc<Self>, app: &Arc<App>, job: &Value) -> Result<()> {
        ...
        let bytes = limits::read_bounded(response, 65536).await?;
...
pub fn router(app: Arc<App>) -> Router {
    Router::new().route("/healthz",get(||async{Json(json!({"status":"ok","runtime":"rust+typescript-pi","ui":"original-svelte"}))}))
        .route("/media/{id}",get(crate::api::local_media))
        .route("/api/image-proxy/{*path}",get(crate::api::image_proxy))
...
async fn forward_inner(app: Arc<App>, request: Request<Body>, management: bool) -> Result<Response> {
    ...
    let bytes = to_bytes(body, 512*1024).await.context("request body exceeds 512 KiB")?;
    ...
    *response.headers_mut().context("response headers")? = headers;
    // Preserve Svelte headers, redirects, every Set-Cookie, and streaming Pi deltas.
