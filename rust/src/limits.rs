...
pub fn client() -> Result<reqwest::Client> {
    Ok(reqwest::Client::builder().user_agent("VTB-Monitor-Rust/0.1")
       .connect_timeout(Duration::from_secs(8)).timeout(Duration::from_secs(40))
       .pool_idle_timeout(Duration::from_secs(30)).pool_max_idle_per_host(1)
...
pub async fn read_bounded(mut response: reqwest::Response, max: usize) -> Result<Vec<u8>> {
    ...
    while let Some(chunk)=response.chunk().await? {
        if out.len().checked_add(chunk.len()).context("size overflow")?>max { bail!("upstream response exceeds byte budget"); }
        out.extend_from_slice(&chunk);
    }
    Ok(out)
}
pub async fn json(response: reqwest::Response) -> Result<serde_json::Value> {
    let status=response.status();
    let bytes=read_bounded(response,JSON_LIMIT).await?;
    if !status.is_success() { bail!("upstream HTTP {}",status.as_u16()); }
    Ok(serde_json::from_slice(&bytes)?)
}
pub fn upstream_stream(response: reqwest::Response, permit: OwnedSemaphorePermit, maximum: usize) -> Body {
    let deadline=tokio::time::Instant::now()+Duration::from_secs(45);
    let body=stream::try_unfold((response.bytes_stream(),permit,0usize,deadline), |(mut upstream,permit,total,deadline)| async move {
        let next=tokio::time::timeout_at(deadline,upstream.next()).await
            .map_err(|_| io::Error::new(io::ErrorKind::TimedOut,"image transfer deadline"))?;
        match next {
            None => Ok(None),
            Some(Err(e))=>Err(io::Error::other(e)),
            Some(Ok(chunk))=>{
                let new_total=total.checked_add(chunk.len()).ok_or_else(||io::Error::other("size overflow"))?;
                if new_total>maximum { return Err(io::Error::other("image exceeds byte budget")); }
                Ok(Some((chunk,(upstream,permit,new_total,deadline))))
            }
        }
    });
...
pub fn file_body(file: tokio::fs::File, permit: OwnedSemaphorePermit) -> Body {
    ...
    let body=stream::unfold((source,permit), |(mut stream,permit)| async move {
        stream.next().await.map(|chunk|(chunk,(stream,permit)))
    });
    Body::from_stream(body)
}
pub fn busy() -> Response {
    Response::builder().status(503).header("retry-after","2").header("cache-control","no-store")
        .body(Body::from("Busy; retry later")).unwrap()
}
...
impl LimitedListener {
    pub fn new(inner: TcpListener, count: usize) -> Self { Self { inner,slots:Arc::new(Semaphore::new(count)) } }
}
            ...
            match self.inner.accept().await {
                Ok((socket,addr))=>{
                    if let Ok(permit)=self.slots.clone().try_acquire_owned() {
                        ...
                        return (LimitedIo{socket,_permit:permit},addr);
                    }
                    drop(socket); // Do not allocate an unbounded queue of waiting connections.
...
impl AsyncRead for LimitedIo {
    fn poll_read(mut self:Pin<&mut Self>,cx:&mut TaskContext<'_>,buf:&mut ReadBuf<'_>)->Poll<io::Result<()>> { Pin::new(&mut self.socket).poll_read(cx,buf) }
}
...
pub async fn image_batch(paths:&[std::path::PathBuf], maximum:usize) -> Result<Vec<Bytes>> {
    ...
    for path in paths {
        ...
        if declared>maximum.saturating_sub(total) as u64 { bail!("image needs derivative/tile; source must remain pending for review"); }
        let mut reader=ReaderStream::with_capacity(file,CHUNK); let mut bytes=Vec::with_capacity(declared as usize);
        while let Some(chunk)=reader.next().await {
            let chunk=chunk?;
            if total+bytes.len()+chunk.len()>maximum { bail!("image changed while reading or exceeds batch budget"); }
            bytes.extend_from_slice(&chunk);
        }
        total+=bytes.len(); output.push(Bytes::from(bytes));
    }
    Ok(output)
}
