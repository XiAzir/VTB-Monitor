...
use vtb_monitor_rs::{api,db,engine,forecast,monitor,hybrid,limits,App};
...
fn main()->Result<()> {
    ...
    runtime.block_on(local.run_until(async move {
        ...
        let predictions=if enabled && app.bridge.is_none() {Some(tokio::task::spawn_local(forecast::run(app.clone())))}else{None};
        let notifications=if enabled {Some(tokio::task::spawn_local(monitor::run(app.clone())))}else{None};
        let web=tokio::spawn(axum::serve(limits::LimitedListener::new(listener,16),if app.bridge.is_some(){hybrid::router(app.clone())}else{api::router(app.clone())}).into_future());
        let management=tokio::spawn(axum::serve(limits::LimitedListener::new(admin_listener,4),if app.bridge.is_some(){hybrid::management_router(app.clone())}else{api::management_router(app.clone())}).into_future());
        ...
        if let Some(bridge) = &app.bridge { bridge.shutdown().await; }
        Ok::<(),anyhow::Error>(())
    }))?;
