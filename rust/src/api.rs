...
use crate::{ai,business,db::{self,*},limits,security,App};
...
pub fn management_router(app:Arc<App>)->Router{
    Router::new().route("/v1/{*path}",any(management_dispatch)).route("/v1/healthz",get(||async{Json(json!({"status":"ok"}))})).with_state(app)
}
...
fn error_response(error:anyhow::Error)->Response{
    ...
    // Do not echo arbitrary DB/provider errors to unauthenticated callers.
    ...
    response.headers_mut().insert("cache-control","no-store".parse().unwrap());if status==503{response.headers_mut().insert("retry-after","2".parse().unwrap());}response
}
#[derive(Clone)]struct Actor{id:String,scopes:Vec<String>,force:bool,session:bool}
impl Actor{fn scope(&self,scope:&str)->Result<()>{if self.session||self.scopes.iter().any(|s|s==scope||s=="*"){Ok(())}else{bail!("FORBIDDEN: missing scope {scope}")}}}
fn cookie(headers:&HeaderMap)->Option<String>{headers.get("cookie")?.to_str().ok()?.split(';').find_map(|part|part.trim().strip_prefix("vtbm_session=").map(String::from))}
async fn actor(app:&Arc<App>,headers:&HeaderMap,management:bool)->Result<Actor>{
    if let Some(token)=headers.get("authorization").and_then(|v|v.to_str().ok()).and_then(|s|s.strip_prefix("Bearer ")){
        if token.len()>256{bail!("AUTH: invalid bearer token");}let hash=security::digest(token);
    ...
    let token=cookie(headers).context("AUTH: login required")?;if token.len()>256{bail!("AUTH: invalid session");}let hash=security::digest(token);
    app.db.call(move|db|{
...
fn csrf(app:&App,headers:&HeaderMap)->Result<()>{
    ...
    if origin!=app.origin{bail!("FORBIDDEN: request Origin does not match configured ORIGIN");}Ok(())
}
...
fn cursor(query:&HashMap<String,String>)->Result<(String,String)>{
    if let Some(c)=query.get("cursor"){if c.len()>256{bail!("invalid cursor");}let parsed:Vec<String>=serde_json::from_slice(&URL_SAFE_NO_PAD.decode(c)?)?;if parsed.len()!=2{bail!("invalid cursor");}Ok((parsed[0].clone(),parsed[1].clone()))}else{Ok(("9999".into(),"~".into()))}
}
...
async fn dispatch_inner(app:Arc<App>,path:String,request:Request<Body>,management:bool)->Result<Response>{
    ...
    if method==Method::GET{
        ...
        let after=cursor(&query)?;let params2=parts.iter().map(|s|s.to_string()).collect::<Vec<_>>();let path2=path.clone();let who2=who.clone();
        let value=app.db.call(move|db|{
            let p:Vec<&str>=params2.iter().map(String::as_str).collect();
            match p.as_slice(){
                ["streamers"]=>{
                    if let Some(w)=&who2{w.scope("config:read")?;}
                    Ok(json!({"data":rows(db,"SELECT s.*,ls.status AS live_status,ls.title AS live_title,ls.checked_at,f.predicted_start_at,f.confidence,f.source AS forecast_source,f.reason AS forecast_reason,f.stale AS forecast_stale FROM streamers s LEFT JOIN live_state ls ON ls.streamer_id=s.id LEFT JOIN forecasts f ON f.id=(SELECT id FROM forecasts WHERE streamer_id=s.id AND active=1 ORDER BY created_at DESC LIMIT 1) WHERE s.id>? ORDER BY s.id LIMIT ?",&[&after.1.replace('~',""),&(limit as i64)],limit)?}))
                },
                ["streamers",sid]=>{let s=one(db,"SELECT s.*,ls.status AS live_status,ls.title AS live_title,ls.checked_at FROM streamers s LEFT JOIN live_state ls ON ls.streamer_id=s.id WHERE s.id=? OR s.slug=? LIMIT 1",&[sid,sid])?;if s.is_null(){bail!("NOT_FOUND: streamer");}let id=strv(&s,"id");Ok(json!({"data":s,"forecast":one(db,"SELECT * FROM forecasts WHERE streamer_id=? AND active=1 ORDER BY created_at DESC LIMIT 1",&[&id])?,"rules":rows(db,"SELECT * FROM schedule_rules WHERE streamer_id=? AND active=1 ORDER BY weekday,local_time LIMIT 100",&[&id],100)?,"exceptions":rows(db,"SELECT * FROM schedule_exceptions WHERE streamer_id=? AND occurrence_date>=date('now','-14 days') ORDER BY occurrence_date DESC LIMIT 100",&[&id],100)?,"evaluations":rows(db,"SELECT * FROM prediction_evaluations WHERE streamer_id=? ORDER BY created_at DESC LIMIT 30",&[&id],30)?}))},
                ["streamers",sid,"dynamics"]=>{let search=query.get("q").cloned().unwrap_or_default();if search.len()>200{bail!("search too long");}let from=query.get("from").cloned().unwrap_or_default();let to=query.get("to").cloned().unwrap_or_else(||"9999".into());let ty=query.get("type").cloned().unwrap_or_default();let state=query.get("state").cloned().unwrap_or_default();let list=rows(db,"SELECT id,streamer_id,type,text,state,published_at,updated_at,source_url,comment_count,like_count,is_pinned,raw_excerpt FROM dynamics WHERE streamer_id=? AND (published_at,id)<(?,?) AND published_at>=? AND published_at<=? AND (?='' OR instr(text,?)>0) AND (?='' OR type=?) AND (?='' OR state=?) ORDER BY published_at DESC,id DESC LIMIT ?",&[sid,&after.0,&after.1,&from,&to,&search,&search,&ty,&ty,&state,&state,&((limit+1)as i64)],limit+1)?;Ok(page(list,limit,"published_at"))},
                ...
                ["jobs"]|["alerts"]|["audit"]|["usage"]|["drafts"]|["reviews"]=>{
                    ...
                    let sql=format!("SELECT * FROM {table} WHERE ({time_key},id)<(?,?) ORDER BY {time_key} DESC,id DESC LIMIT ?");Ok(page(rows(db,&sql,&[&after.0,&after.1,&((limit+1)as i64)],limit+1)?,limit,time_key))
                },
                ...
                ["chat",conversation]=>{let who=who2.as_ref().unwrap();who.scope("ops:run")?;let key=format!("admin_v2:{}:{conversation}",who.id);Ok(page(rows(db,"SELECT * FROM pi_messages WHERE conversation_id=? AND (created_at,id)<(?,?) ORDER BY created_at DESC,id DESC LIMIT ?",&[&key,&after.0,&after.1,&((limit+1)as i64)],limit+1)?,limit,"created_at"))},
    ...
    if path=="logout"{if let Some(token)=cookie(&headers){let hash=security::digest(token);app.db.call(move|db|{db.execute("DELETE FROM admin_sessions WHERE token_hash=?",[hash])?;Ok(())}).await?;}let mut response=Json(json!({"ok":true})).into_response();response.headers_mut().insert("set-cookie","vtbm_session=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0".parse()?);return Ok(response);}
    if path=="password"{if !who.session{bail!("FORBIDDEN: password change requires admin session");}let password=business::required(&body,"password",1024)?.to_owned();let permit=app.expensive.clone().try_acquire_owned().map_err(|_|anyhow::anyhow!("BUSY: expensive work in progress"))?;let encoded=tokio::task::spawn_blocking(move||{let _permit=permit;security::hash_password(&password)}).await??;let actor=who.id;app.db.call(move|db|{let tx=db.transaction()?;tx.execute("UPDATE admins SET password_hash=?,force_password_change=0,updated_at=? WHERE id=?",params![encoded,now(),actor])?;tx.execute("DELETE FROM admin_sessions WHERE admin_id=?",[actor])?;tx.commit()?;Ok(())}).await?;return Ok(Json(json!({"ok":true,"loginAgain":true})).into_response());}
    if path=="chat"{who.scope("ops:run")?;let prompt=business::required(&body,"prompt",16000)?;let conversation=business::required(&body,"conversationId",64)?;let answer=ai::admin_chat(&app,&who.id,conversation,prompt).await?;return Ok(Json(json!({"answer":answer})).into_response());}
    ...
    let signature=security::digest(json!([method.as_str(),path,body]).to_string());let key=app.key;let path=path.clone();
    let result=app.db.call(move|db|{
        let p:Vec<&str>=path.split('/').collect();
        if let Some(k)=&idem{let old=one(db,"SELECT request_hash,result FROM rs_idempotency WHERE key=? AND actor=?",&[k,&who.id])?;if !old.is_null(){if strv(&old,"request_hash")!=signature{bail!("CONFLICT: idempotency key reused with different request");}return Ok(serde_json::from_str(strv(&old,"result"))?);}}
        let result=match p.as_slice(){
            ...
            ["streamers",sid,"operations",operation]=>{if one(db,"SELECT id FROM streamers WHERE id=?",&[sid])?.is_null(){bail!("NOT_FOUND: streamer");}let kind=match *operation{"sync"|"refresh"=>"sync_streamer","reanalyze"=>"pi_analyze","reforecast"=>{let id=business::refresh_forecast(db,sid)?;return Ok(json!({"forecastId":id}));},_=>bail!("unknown operation")};json!({"jobId":enqueue(db,kind,sid,json!({"fullSync":*operation=="refresh","instruction":body["instruction"]}),10,0,&format!("operation:{sid}:{}",idem.clone().unwrap_or_else(id)))?})},
            ["dynamics",did,"operations","refresh"]=>json!({"jobId":enqueue(db,"refresh_dynamic",did,json!({}),10,0,&format!("refresh:{did}:{}",idem.clone().unwrap_or_else(id)))?}),
            ["streamers",sid,"rules"]=>{business::replace_rules(db,sid,body["rules"].as_array().context("rules must be array")?,&who.id)?;json!({"ok":true})},
            ["streamers",sid,"forecast"]=>{let tx=db.transaction()?;let fid=business::set_forecast(&tx,sid,business::required(&body,"predictedStartAt",50)?,"manual",body["reason"].as_str().unwrap_or("人工预测"),number(&body,"confidence",100),body["evidence"].clone())?;audit(&tx,&who.id,"forecast.manual","forecast",&fid,json!({"streamerId":sid}))?;tx.commit()?;json!({"id":fid})},
            ["drafts",did,"confirm"]=>json!({"confirmed":business::confirm_draft(db,did,body["monday"].as_str(),&who.id)?}),
            ["drafts",did,"entries"]=>{let entries:Vec<crate::time::Entry>=serde_json::from_value(body["entries"].clone())?;if entries.len()>70{bail!("too many entries");}let n=db.execute("UPDATE schedule_drafts SET entries_json=?,status='review',updated_at=? WHERE id=? AND status NOT IN ('confirmed','rejected')",params![serde_json::to_string(&entries)?,now(),did])?;if n==0{bail!("CONFLICT: draft not editable");}json!({"ok":true})},
            ...
            ["settings",name]=>{if !["pi_profile","smtp","bilibili_proxy_url"].contains(name){bail!("unsupported setting");}let value=body["value"].to_string();if value.len()>16384{bail!("setting too large");}db.execute("INSERT INTO settings(key,value_json,updated_at) VALUES(?,?,?) ON CONFLICT(key) DO UPDATE SET value_json=excluded.value_json,version=settings.version+1,updated_at=excluded.updated_at",params![name,value,now()])?;audit(db,&who.id,"setting.write","setting",name,json!({"key":name}))?;json!({"ok":true})},
            ["secrets",name]=>{if !["pi_api_key","bilibili_cookie","smtp_password"].contains(name)&&!name.starts_with("bilibili_cookie_pool:"){bail!("unsupported secret name");}let value=security::encrypt(&key,business::required(&body,"value",65536)?)?;db.execute("INSERT INTO secrets(key,encrypted_value,status,updated_at) VALUES(?,?,'untested',?) ON CONFLICT(key) DO UPDATE SET encrypted_value=excluded.encrypted_value,status='untested',updated_at=excluded.updated_at",params![name,value,now()])?;audit(db,&who.id,"secret.write","secret",name,json!({"value":"[REDACTED]"}))?;json!({"ok":true})},
            ["tokens"]=>{let scopes=body["scopes"].as_array().context("scopes required")?;if scopes.len()>10||scopes.iter().any(|s|!matches!(s.as_str(),Some("config:read"|"config:write"|"status:read"|"audit:read"|"ops:run"|"secrets:read"|"secrets:write"))){bail!("invalid scopes");}let token=security::token("vtbm");let tid=id();db.execute("INSERT INTO api_tokens(id,name,token_prefix,token_hash,scopes_json,expires_at,created_at) VALUES(?,?,?,?,?,?,?)",params![tid,business::required(&body,"name",100)?,&token[..12],security::digest(&token),json!(scopes).to_string(),body["expiresAt"].as_str(),now()])?;json!({"id":tid,"token":token})},
            ["tokens",tid,"revoke"]=>{db.execute("UPDATE api_tokens SET revoked_at=? WHERE id=?",params![now(),tid])?;json!({"ok":true})},
            _=>bail!("NOT_FOUND: API route"),
        };
        if let Some(k)=idem{db.execute("INSERT INTO rs_idempotency VALUES(?,?,?,?,?)",params![k,who.id,signature,result.to_string(),now()])?;}
        Ok(result)
    }).await?;Ok(Json(result).into_response())
}
...
async fn local_media(State(app):State<Arc<App>>,Path(mid):Path<String>)->Response{
    ...
    }.await;match result{Ok(r)=>r,Err(e)=>error_response(e)}
}
